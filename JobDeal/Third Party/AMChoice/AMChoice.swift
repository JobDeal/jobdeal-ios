...
class AMChoice: UIView,UITableViewDelegate,UITableViewDataSource {
    ...
    final func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.imgRadio.tintColor = UIColor.darkGray
    ...
    final func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let lastIndexPathh = lastIndexPath{
            if lastIndexPath?.row ?? 0 < data.count {
                tableView.reloadRows(at: [lastIndexPathh], with: .automatic)
            }
        }
