...
class AJAlertController: UIViewController {
    ...
    private var strAlertTitle = "Job Deal"
    ...
    private func setupAJAlertController() {
        ...
        if #unavailable(iOS 13) {
            imageView.image = UIImage(named: "logoAlertIcon")?.tintWithColor(.black)
        } else {
            imageView.image = UIImage(named: "logoAlertIcon")?.withRenderingMode(.alwaysTemplate)
    ...
    private func preferredAlertWidth()
    {
        switch UIDevice.current.userInterfaceIdiom {
            ...
            case .pad:                alertWidthConstraint.constant = 340.0
        case .mac: break
